...
class_name EffectHunterSuppressiveShot
...
@export var shot_interval_sec: float = 0.06
...
func apply(caster: Node, target: Node, rank_data: RankData, context: Dictionary) -> void:
	...
	var targets: Array[Node2D] = _collect_targets(caster as Node2D, primary_target, cast_range, limit)
	if targets.is_empty():
		return
	...
	for i in range(targets.size()):
		...
		if t == null or not is_instance_valid(t):
			continue
		if "is_dead" in t and bool(t.get("is_dead")):
			continue
		var final_damage: int = STAT_CALC.apply_crit_to_damage_typed(base_damage, snap, school)
		if not _spawn_projectile(caster as Node2D, t, final_damage):
			DAMAGE_HELPER.apply_damage_typed_with_result(caster, t, final_damage, school)
		if i < targets.size() - 1:
			await (caster as Node2D).get_tree().create_timer(max(0.01, shot_interval_sec)).timeout

func _spawn_projectile(caster: Node2D, target: Node2D, damage: int) -> bool:
	if projectile_scene == null or caster == null or target == null:
		return false
...
func _distance_between_body_hitboxes(a: Node2D, b: Node2D) -> float:
	if a == null or b == null:
		return INF
	var a_pos: Vector2 = _resolve_anchor(a)
	var b_pos: Vector2 = _resolve_anchor(b)
	return a_pos.distance_to(b_pos)

func _collect_targets(caster: Node2D, primary_target: Node2D, cast_range: float, limit: int) -> Array[Node2D]:
	var out: Array[Node2D] = []
	if caster == null or caster.get_tree() == null:
		return out

	if primary_target != null and _is_valid_enemy_target(caster, primary_target):
		if _distance_between_body_hitboxes(caster, primary_target) <= cast_range:
	...
	for node in nodes:
		...
		if not _is_valid_enemy_target(caster, unit):
			continue
		var dist: float = _distance_between_body_hitboxes(caster, unit)
		if dist > cast_range:
			continue
