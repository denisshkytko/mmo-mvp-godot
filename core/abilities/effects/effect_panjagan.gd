...
@export var shot_interval_sec: float = 0.08
...
func apply(caster: Node, target: Node, rank_data: RankData, context: Dictionary) -> void:
	...
	_run_projectile_sequence(caster as Node2D, target as Node2D, base_damage, snap)

func _run_projectile_sequence(caster: Node2D, target: Node2D, base_damage: int, snap: Dictionary) -> void:
	if caster == null or not is_instance_valid(caster):
		return
	var total_shots: int = max(1, hit_count)
	for i in range(total_shots):
		...
		if not _spawn_projectile(caster, target, hit_damage):
			DAMAGE_HELPER.apply_damage_typed_with_result(caster, target, hit_damage, school)

		if i < total_shots - 1:
			await caster.get_tree().create_timer(max(0.01, shot_interval_sec)).timeout
